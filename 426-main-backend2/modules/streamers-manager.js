...
MongoClient.connect(process.env.DB_URL, { useNewUrlParser: true }, function(e, client) {
	...
	// index fields 'user' & 'email' for faster new account validation //
		streamers.createIndex({streamer_name: 1, streamer_id: 1});
...
{
	streamers.findOne({user:user}, function(e, o) {
		if (o){
...
{
	let cookie = guid();
	streamers.findOneAndUpdate({user:user}, {$set:{
		ip : ipAddress,
		cookie : cookie
...
// ensure the cookie maps to the user's last recorded ip address //
	streamers.findOne({cookie:cookie, ip:ipAddress}, callback);
}

exports.generatePasswordKey = function(email, ipAddress, callback)
{
	let passKey = guid();
	streamers.findOneAndUpdate({email:email}, {$set:{
		ip : ipAddress,
		passKey : passKey
...
// ensure the passKey maps to the user's last recorded ip address //
	streamers.findOne({passKey:passKey, ip:ipAddress}, callback);
}
...
exports.addNewAccount = function(newData, callback)
{
    ...
    streamers.insertOne(newData, callback);
...
exports.updateAccount = function(newData, callback)
{
	let findOneAndUpdate = function(data){
		...
		streamers.findOneAndUpdate({_id:getObjectId(data.id)}, {$set:o}, {returnOriginal : false}, callback);
	}
	if (newData.pass == ''){
...
exports.updatePassword = function(passKey, newPass, callback)
{
	saltAndHash(newPass, function(hash){
		newPass = hash;
		streamers.findOneAndUpdate({passKey:passKey}, {$set:{pass:newPass}, $unset:{passKey:''}}, {returnOriginal : false}, callback);
	});
}
...
exports.actLookup = function(key, callback) {
	streamers.findOne({activation_key: key}, function(e, w) {
		if (e) callback(e)
		else callback(null, w);
	})
}

exports.usernameLookup = function(username, callback) {
	streamers.findOne({user: username}, function(e, q) {
		if (e) callback(e)
		else callback(null, q);
	})
}
...
{
	streamers.findOne({_id: getObjectId(id)}, function(e, q) {
		if (e) {
	...
	streamers.deleteOne({_id: getObjectId(id)}, callback);
}
exports.deleteAccountByUser = function(id, callback)
{
	streamers.deleteOne({user: id}, callback);
}
exports.deleteAllAccounts = function(callback)
{
	streamers.deleteMany({}, callback);
}
...
{
	streamers.indexes(null, function(e, indexes){
